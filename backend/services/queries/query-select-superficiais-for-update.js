const querySelectSuperficiaisForUpdate = (ids)=> {
    ...
    */
    ...
    SUBSTRING(REPLACE(CAST(ROUND(E.VAZAO,2) AS VARCHAR),'.',','),1,LEN(E.VAZAO)-1) AS DAD_QT_VAZAODIAJAN,
    SUBSTRING(REPLACE(CAST(ROUND(F.VAZAO,2) AS VARCHAR),'.',','),1,LEN(F.VAZAO)-1) AS DAD_QT_VAZAODIAFEV,
    SUBSTRING(REPLACE(CAST(ROUND(G.VAZAO,2) AS VARCHAR),'.',','),1,LEN(G.VAZAO)-1) AS DAD_QT_VAZAODIAMAR,
    SUBSTRING(REPLACE(CAST(ROUND(H.VAZAO,2) AS VARCHAR),'.',','),1,LEN(H.VAZAO)-1) AS DAD_QT_VAZAODIAABR,
    SUBSTRING(REPLACE(CAST(ROUND(I.VAZAO,2) AS VARCHAR),'.',','),1,LEN(I.VAZAO)-1) AS DAD_QT_VAZAODIAMAI,
    SUBSTRING(REPLACE(CAST(ROUND(J.VAZAO,2) AS VARCHAR),'.',','),1,LEN(J.VAZAO)-1) AS DAD_QT_VAZAODIAJUN,
    SUBSTRING(REPLACE(CAST(ROUND(L.VAZAO,2) AS VARCHAR),'.',','),1,LEN(L.VAZAO)-1) AS DAD_QT_VAZAODIAJUL,
    SUBSTRING(REPLACE(CAST(ROUND(M.VAZAO,2) AS VARCHAR),'.',','),1,LEN(M.VAZAO)-1) AS DAD_QT_VAZAODIAAGO,
    SUBSTRING(REPLACE(CAST(ROUND(N.VAZAO,2) AS VARCHAR),'.',','),1,LEN(N.VAZAO)-1) AS DAD_QT_VAZAODIASET,
    SUBSTRING(REPLACE(CAST(ROUND(O.VAZAO,2) AS VARCHAR),'.',','),1,LEN(O.VAZAO)-1) AS DAD_QT_VAZAODIAOUT,
    SUBSTRING(REPLACE(CAST(ROUND(P.VAZAO,2) AS VARCHAR),'.',','),1,LEN(P.VAZAO)-1) AS DAD_QT_VAZAODIANOV,
    SUBSTRING(REPLACE(CAST(ROUND(Q.VAZAO,2) AS VARCHAR),'.',','),1,LEN(Q.VAZAO)-1) AS DAD_QT_VAZAODIADEZ,
    ...
    CASE 
        ...
        THEN 5
        WHEN S.ID_TIPO_FINALIDADE = 5 AND (SUBFINALIDADE NOT LIKE '%LAVAG%' AND SUBFINALIDADE NOT LIKE '%LAVANDERIA%')
        THEN 99 --16 Não pode adicionar este tipo de finalidade, 16
    ...
    -- Ao editar trazer todos para atualizar a situação no SNRIH
